...
static void get_mouse_press_msg(shoot_control_data_t *shoot)	//**
{
	...
	{
		shoot->mouse_left_single_click_flag = 1;//单发标志位
	}
	else
	{
	...
	if(shoot->rc_ctrl->mouse.press_r)
	{
		...
		{
			shoot->mouse_right_long_press_flag = 1;						//连射
		}
	}
	else
	{
...
static void get_shoot_related_flags(shoot_control_data_t *shoot)	//**
{
	...
	if(control_mode == KEY_MOUSE_MODE)				//键鼠
	{
		get_mouse_press_msg(shoot);
	}
	else if(control_mode == REMOTE_MODE)			//遥控
	{
...
/**
  ...
  * @note           
  */
static void shoot_mode_update(shoot_control_data_t *shoot, shoot_pid_t *shoot_pid)//射击状态变化	***
{
...
//		}
	...
	shoot->fric1_fdb = (float)shoot->fric_motor1_msg->encoder.filter_rate;//电机速度
...
static void shoot_cascade_pid_calculate(shoot_pid_t *shoot_pid, shoot_control_data_t *shoot)//***
{
	...
	shoot_pid->trigger_pid.position_pid.Calc(&shoot_pid->trigger_pid.position_pid);							//位置环
	...
	shoot_pid->trigger_pid.speed_pid.Calc(&shoot_pid->trigger_pid.speed_pid);										//速度环
	...
	shoot_pid->fric_1_pid.Calc(&shoot_pid->fric_1_pid);																			  	//位置环
	                                                                                            
	shoot->given_current.fric1_motor = shoot_pid->fric_1_pid.output;                            
	//fric2 pid                                                                                 
	shoot_pid->fric_2_pid.set = shoot->fric2_set;                                               //速度环
...
void shoot_task(void *argument)
{
	...
	{	
		...
		shoot_limit();                                          // 规则限制
		get_shoot_related_flags(&shoot_control_data);						//获取数据
		shoot_mode_update(&shoot_control_data, &shoot_pid);			//模式更新
		shoot_set_and_fdb_update(&shoot_control_data);					//pid值更新
		shoot_cascade_pid_calculate(&shoot_pid, &shoot_control_data);			//pid计算
		shoot_control_loop(&shoot_control_data); 								//不断输出电流值                   
