...
uint8_t can1_rx_data[CAN_TX_BUF_SIZE];
CAN_TxHeaderTypeDef can1_tx_header;
uint8_t can1_tx_data[CAN_RX_BUF_SIZE];
CAN_RxHeaderTypeDef can2_rx_header;
uint8_t can2_rx_data[CAN_TX_BUF_SIZE];
CAN_TxHeaderTypeDef can2_tx_header;
uint8_t can2_tx_data[CAN_RX_BUF_SIZE];
/* USER CODE END 0 */
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    if(hcan->Instance == CAN1)// can1
    {
      ...
      can1_message_progress(&can1_rx_header, can1_rx_data); 
    }
    if(hcan->Instance == CAN2)// can2
    {
      HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &can2_rx_header, can2_rx_data);
      can2_message_progress(&can2_rx_header, can2_rx_data); 
    }
}
